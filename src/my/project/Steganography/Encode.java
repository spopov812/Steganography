...
/**
 * 
 * Blueprint for a object that can encode a steganographic
 * image
 * 
...
public class Encode {
	...
	/**
	 * 
	 * Constructor that takes a text file and encodes it into an image's
	 * pixel, altering the image imperceptibly. It takes 4 pixels
	 * to encode 3 bytes (characters). Each pixel can store 6 bits of encoded
	 * data (2 for each of the rgb values)
	 * 
	 * @param imageStr image into which data will be encoded. The last two bits of
	 * some of the pixels rgb data will be changed to encode text.
	 * 
	 * @param file text file that will be encoded in the pixel data of the image
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * 
	 * Determines if there are enough pixels in the image to encode
	 * the file
	 * 
	...
	/**
	 * 
	 * Calls two methods that encode the text file size in the image 
	 * and then actually encodes the text data into the image with a 
	 * proper interval between the pixels. Also prints the final steganographic png
	 * 
	 */
	...
	/**
	 * 
	 * Encodes the size of the text file into the first 6
	 * pixels.
	 * 
	 */
	...
	/**
	 * 
	 * Writes actual text file into the image after the first
	 * 6 pixels with a proper interval
	 * 
	 */
		...
		for (int i = 0; i < fileAsBytes.length; i++){
			...
			if (iterator == 3){
				...
				for (int j = 0; j < 4; j++){
					...
					//encodes proper values into last two bits of rgb pixel data
					red |= bitsToEncode[lastArrayIndex];
					lastArrayIndex++;
